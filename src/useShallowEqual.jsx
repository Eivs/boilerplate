 ...
 * This will return true for unchanged props
 * (where the only changes are the react elements props like 'children')
...
export function shallowEqualWithoutReactElements(thisProps, nextProps) {
  ...
  if (thisProps === nextProps) {
    ...
    for (const propName of propNames) {
      if (
        thisProps[propName] !== nextProps[propName]
        && !isReactElement(thisProps[propName])
      ) {
...
export function useShallowEqual(WrappedComponent) {
  ...
  ShallowEqualEnhancer.displayName = `ShallowEqualEnhanced${WrappedComponent.displayName
